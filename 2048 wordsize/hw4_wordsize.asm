...
place:
	...
	lw $t6 4($sp)			# Get the value of val
	...
	validPlace:			# a[i][j] = base address + (i*n_col+j) * size	t5 = col = j	t6 = val
		...
		sll $t0 $t0 2		# [(i*n_col) + j] * 4	size = 4
		add $t0 $t0 $a0		# a[i][j]
		sw $t7 ($t0)		# put the val in index
...
merge_row:
	...
	rightToLeft:
		bltz $s4 doneMergeRow	# Stop when s4 = 0. We dont count the last value.

		# base address + (i*n_col+j) * size
		mul $t0 $s3 $s2		# i * n_col
		add $t1 $t0 $s4		# i*n_col+j	right value
		sll $t1 $t1 2		# (i*n_col+j)*size
		add $t1 $t1 $s0		# a[i][j] = base address + (i*4+j) * size
		lw $t1 ($t1)		# The value at these two locations
		...
		sll $t0 $t0 2		# (i*n_col+j)*size
		add $t0 $t0 $s0		# a[i][j] = base address + (i*4+j) * size
		lw $t0 ($t0)		# The value at these two locations
	...
	leftToRight:
		...
		sll $t0 $t0 2		# (row*n_cols+col) * 4 <- 4 is the size
		add $t0 $t0 $s0		# a[i][j] <- the left one
		lw $t0 ($t0)		# element at that index
		...
		sll $t1 $t1 2		# (row*n_cols+col) * 4 <- 4 is the size
		add $t1 $t1 $s0		# a[i][j] <- the right one
		lw $t1 ($t1)		# element at that value
...
merge_col:
	...
	addi $t0 $a1 -1		# index = n_row - 1
		...
		mul $t3 $t0 $a2		# i * n_col
    		...
    		sll $t3 $t3 2		# mul 4
    		add $t3 $t3 $a0		# a[i][j]
		lw $t4 ($t3) 		# element at index

		bne $t2 $t4 bottomToTop
		sll $t2 $t2 1		# value * 2
		sw $t2 ($t1)		# replace value with double
		li $t4 -1
		sw $t4 ($t3)		# replace top element with -1
		...
		mul $t2 $t0 $a2		# i * n_col
    		...
    		sll $t2 $t2 2		# mul 4
    		add $t2 $t2 $a0		# a[i][j]
		lw $t4 ($t2) 		# element at index

		bne $t3 $t4 topToBottom
		sll $t3 $t3 1		# value * 2
		sw $t3 ($t1)		# replace value with double
		li $t3 -1
		sw $t3 ($t2)		# replace bottom index with -1
		addi $t0 $t0 1		# Skip the next one. It's -1 so...
		j topToBottom
...
shift_row:
	...
	moveRight:
		li $t6 -1
	    	sw $t6 ($t2)		# -1 at this location
	    	sw $t3 ($t4)		# Move to this location

	    	sub $t4 $t4 $t2		# End location - start location
	    	srl $t4 $t4 2		# Getting the amt shifted
	    	add $v0 $v0 $t4
	    	j dirRight
